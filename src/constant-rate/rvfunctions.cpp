...
double GenerateMutationProb(MutationParameters mut_params)
{
  ...
  {
    mut_prob = gsl_ran_beta(gp.rng, mut_params.alpha_mutation, mut_params.beta_mutation);
  }
...
int GeneratePunctuation(PunctuationParameters punct_params)
{
  ...
  double rand_pois = gsl_ran_flat(gp.rng, 0, 1);
  rand_pois = -log(1 - rand_pois * (1 - exp(-lambda)));
  rand_pois = lambda - rand_pois;

  int zero_trun_pois = gsl_ran_poisson(gp.rng, rand_pois) + 1;
